...
Item {
  ...
  component BarPanel: PanelWindow {
    ...
    margins {
      top: root.barHidden && root.position === "top"
          ? -30
          : (root.position === "top" ? 10 : 0)

      bottom: root.barHidden && root.position === "bottom"
          ? -30
          : (root.position === "bottom" ? 10 : 0)

      left: root.barHidden && root.position === "left"
          ? -root.barSize
          : (root.position === "left" ? 0 : Math.max(0, Math.round((screen.width - 1750) / 2)))

      right: root.barHidden && root.position === "right"
          ? -root.barSize
          : (root.position === "right" ? 0 : Math.max(0, Math.round((screen.width - 1750) / 2)))
    }
    ...
    implicitWidth: root.vertical ? root.barSize : 1750
    implicitHeight: root.vertical ? 0 : 50
    ...
    Rectangle {
      ...
      radius: 20
      color: Qt.rgba(0.102, 0.106, 0.149, 0.90)
  ...
  component ModuleSlot: Item {
    ...
    ? (root.vertical ? activeItem.implicitHeight : Math.min(activeItem.implicitHeight, 30))
    : 0
    width: implicitWidth
    ...
    readonly property bool noPill: ["omarchy.menu", "omarchy.tray", "omarchy.indicators", "caste.menu", "caste.tray", "caste.dots"].indexOf(slot.moduleName) !== -1
